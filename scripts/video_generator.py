...
ENGLISH_FONT_SIZE  = 34           # pt 1 – bigger
...
def generate_ayah_videos(ayah: dict, surah_name: str, surah_number: int,
    ...
    if not audio_path:
        print(f"   ✗ Could not download audio for ayah {ayah['number']}")
        return
    ...
    yt_clip.close()
    print(f"   ✓ YouTube video: {yt_out}  ({total_dur:.1f}s)")
    ...
    if total_dur <= REEL_MAX_DURATION:
        ...
        for part_idx in range(num_parts):
            ...
            part_clip.close()
            print(f"   ✓ Reel part {part_idx+1}/{num_parts}: {reel_out}")
...
def generate_all():
    ...
    print(f"\n📖  Surah: {surah_name}  ({len(surah['ayahs'])} ayahs)")
    print(f"🖼️   Backgrounds available: {len(backgrounds)}")
    ...
    print("   📱  Reels  (≤120 s)  → videos/reels/")
    print("   ▶️   YouTube (full)   → videos/youtube/")
